...
router.get('/settings', authenticate, authorizeRoles('owner', 'admin'), async (req, res) => {
  try {
...
router.put('/settings', authenticate, authorizeRoles('owner', 'admin'), async (req, res) => {
  try {
    const {
...
router.post('/', authenticate, authorizeRoles('owner', 'admin', 'manager'), async (req, res) => {
  try {
    const {
      clientId, invoiceType = 'one_time', items, taxRate = 0, discountAmount = 0,
      notes, terms, issueDate, dueDate, billingPeriodStart, billingPeriodEnd,
    ...
    const invoiceResult = await query(
      `INSERT INTO invoices (
        ...
        issue_date, due_date, notes, terms, billing_period_start, billing_period_end,
        adjustment_description, adjustment_amount,
        status, created_by
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, $9, $10, $11, $12, $13, $14, $15, $16, $17, 'draft', $18)
      RETURNING *`,
      [
        req.user.company_id, clientId, invoiceNumber, invoiceType,
        subtotal, taxRate, taxAmount, discountAmount, total,
        finalIssueDate, finalDueDate, notes, terms, billingPeriodStart, billingPeriodEnd,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await query(
        `INSERT INTO invoice_items (invoice_id, description, quantity, unit_price, amount, service_item_id, sort_order)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [invoice.id, item.description, item.quantity, item.unitPrice, amount, item.serviceItemId || null, i]
      );
    }
...
router.put('/:id', authenticate, authorizeRoles('owner', 'admin', 'manager'), async (req, res) => {
  try {
...
router.post('/:id/send', authenticate, authorizeRoles('owner', 'admin', 'manager'), async (req, res) => {
  try {
...
router.post('/:id/payment', authenticate, authorizeRoles('owner', 'admin', 'manager'), async (req, res) => {
  try {
    ...
    const paymentResult = await query(
      ...
      RETURNING *`,
      [
        req.user.company_id, invoice.client_id, req.params.id, amount,
        paymentMethod || 'cash', referenceNumber, notes,
        paymentDate || new Date(), req.user.id
      ]
    );
...
router.post('/generate/:clientId', authenticate, authorizeRoles('owner', 'admin'), async (req, res) => {
  try {
    ...
    const invoiceResult = await query(
      ...
      RETURNING *`,
      [
        req.user.company_id, req.params.clientId, invoiceNumber,
        subtotal, taxRate, taxAmount, total,
        dueDate.toISOString().split('T')[0], periodStart, periodEnd,
        settings.default_terms, settings.default_notes, req.user.id
      ]
    );
...
router.post('/generate-monthly', authenticate, authorizeRoles('owner', 'admin'), async (req, res) => {
  try {
    ...
    for (const client of clientsResult.rows) {
      try {
        ...
        const existing = await query(
          ...
          [client.id, req.user.company_id, billingPeriodStart]
        );
        ...
        const invoiceResult = await query(
          ...
          RETURNING *`,
          [
            req.user.company_id, client.id, invoiceNumber,
            subtotal, taxRate, taxAmount, total,
            dueDate.toISOString().split('T')[0], billingPeriodStart, billingPeriodEnd,
            autoSend ? 'sent' : 'draft', req.user.id
          ]
        );
...
router.post('/mark-overdue', authenticate, authorizeRoles('owner', 'admin'), async (req, res) => {
  try {
