...
function InvoiceSettingsModal({
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'template' | 'automation'>('general');
  ...
  const fetchSettings = async () => {
    try {
      const res = await axios.get(`${apiUrl}/invoices/settings`, getAuthHeaders());
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await axios.put(`${apiUrl}/invoices/settings`, {
        invoicePrefix: settings.invoice_prefix,
        estimatePrefix: settings.estimate_prefix,
        ...
        autoGenerateMonthly: settings.auto_generate_monthly,
        autoGenerateDay: settings.auto_generate_day,
      }, getAuthHeaders());
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateSetting('estimate_prefix', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                    placeholder="EST-"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateSetting('default_due_days', parseInt(e.target.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                    min="1"
                  />
                </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                  placeholder="Ingrese los términos que aparecerán en cada factura..."
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                  placeholder="Puede pagar mediante: ATH Móvil, transferencia bancaria..."
                />
              </div>
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                ...
                <div className="bg-white border rounded-lg p-4 text-sm">
                  <div className="border-b pb-2 mb-2">
                    <strong>FACTURA #{settings.invoice_prefix}00001</strong>
                  </div>
                  {settings.default_terms && (
                    <div className="mb-2">
                      <strong>Términos:</strong>
                      <p className="text-gray-600 whitespace-pre-wrap">{settings.default_terms}</p>
                    </div>
                  )}
                  {settings.payment_instructions && (
                    <div className="mb-2">
                      <strong>Instrucciones de Pago:</strong>
                      <p className="text-gray-600 whitespace-pre-wrap">{settings.payment_instructions}</p>
                    </div>
                  )}
                  {settings.default_notes && (
                    <div>
                      <strong>Notas:</strong>
                      <p className="text-gray-600 whitespace-pre-wrap">{settings.default_notes}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'automation' && settings && (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  <h4 className="font-medium">Generar Facturas Automáticamente</h4>
                  <p className="text-sm text-gray-500">
                    Crear facturas mensuales para clientes con servicio recurrente
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={settings.auto_generate_monthly || false}
                    onChange={(e) => updateSetting('auto_generate_monthly', e.target.checked)}
              ...
              {settings.auto_generate_monthly && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Día del Mes para Generar
                  </label>
                  <select
                    value={settings.auto_generate_day || 1}
                    onChange={(e) => updateSetting('auto_generate_day', parseInt(e.target.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500"
                  >
...
export default function InvoicesPage() {
  ...
  const handleSendWhatsApp = () => {
    ...
    const clientName = `${selectedInvoice.client_first_name || ''} ${selectedInvoice.client_last_name || ''}`.trim();
    const message = encodeURIComponent(
      `Hola ${clientName}! Le enviamos la factura ${selectedInvoice.invoice_number} por ${formatCurrency(selectedInvoice.total)}. ` +
      `Vence el ${formatDate(selectedInvoice.due_date)}. ` +
      `Pendiente: ${formatCurrency(selectedInvoice.balance_due)}. ¡Gracias!`
    );

    // Clean phone number (remove spaces, dashes, etc)
    let clientPhone = selectedInvoice.client_phone || '';
    clientPhone = clientPhone.replace(/[\s\-\(\)]/g, '');

    // Add country code if not present (default to Puerto Rico +1)
    if (clientPhone && !clientPhone.startsWith('+') && !clientPhone.startsWith('1')) {
    ...
    if (clientPhone && clientPhone.length >= 10) {
      ...
      alert('El cliente no tiene teléfono configurado. El mensaje fue copiado al portapapeles.');
    }
  };
